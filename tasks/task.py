...
class Task:
    ...
    def make_optimizer(self, model=None, epoch=0, hlpr=None, attack=False) -> Optimizer:
        if model is None:
            model = self.model

        if attack:
            lr = self.params.lr * self.params.gamma ** (epoch)
        else:
            lr = self.params.lr * self.params.gamma ** (epoch)
    ...
    def make_anticipate_optimizer(self, params, epoch=0) -> Optimizer:
        lr = self.params.lr * self.params.gamma ** (epoch)
