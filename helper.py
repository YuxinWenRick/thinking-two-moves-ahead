...
class Helper:
    ...
    def __init__(self, params):
        ...
        if 'spectral_evasion' in self.params.loss_tasks \
            or 'latent' in self.params.loss_tasks \
            or 'feature' in self.params.loss_tasks:
    ...
    def make_synthesizer(self):
        if self.params.synthesizer is None or self.params.synthesizer == 'None':
    ...
    def make_folders(self):
        ...
        if self.params.tb:
            # wr = SummaryWriter(log_dir=f'runs/{self.params.name}_{self.params.current_time}')
    ...
    def report_training_losses_scales(self, batch_id, epoch, extra_info=None, fl_attacker=False):
        if fl_attacker:
        ...
        if extra_info is not None:
            for name, values in extra_info.items():
                self.plot(epoch * total_batches + batch_id, values,
                        f'Train/{name}')
    ...
    def fix_random(seed=1):
        ...
        logger.info('Setting random_seed seed for reproducible results.')
        random.seed(seed)
